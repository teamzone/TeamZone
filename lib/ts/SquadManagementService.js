    ...
    var SquadManagementService = (function (_super) {
        ...
        function SquadManagementService(_squads, _players, _squadplayers) {
                ...
                _this.GetPlayersForSquad(clubname, cityname, squadname, season, function (err, squadplayers) {
                    if (err) {
                        ...
                        playersdb.createReadStream({ gte: clubKey })
                            ...
                            if (!_.find(squadplayers, function (s) { return s.email === player.email; })) {
                                players.push(player);
                            }
                        })
                            .on('end', function () {
            ...
            this.GetPlayersForSquad = function (clubname, cityname, squadname, season, callback) {
                var players = [];
                _this._squadplayers.createReadStream(kf.KeyFactory.squadKeyMaker(clubname, cityname, squadname, season))
                    .on('data', function (player) {
                    players.push(player);
                })
                    .on('end', function () {
