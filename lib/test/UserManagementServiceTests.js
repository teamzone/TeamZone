...
describe("User Management Service Unit Tests", function() {
  ...
  it('Returns error when user not longer exists prior to confirming', function(done) {
    ...
    //3. Verify
    assertErrorOnGetForRegisteredUser(user, 'The user ' + user + ' is not present in the database');
    done();
  });
  ...
  it('Returns error on a general get failure prior to confirming', function(done) {
    ...
    //3. Verify
    assertErrorOnGetForRegisteredUser(user, 'A failure occurred trying to retrieve details for ' + user);
    done();
  });
  ...
  it('Returns error on when saving the confirmed user fails', function(done) {
    ...
    //3. Verify
    assertErrorOnSaveForRegisteredUser(user, 'A failure occurred trying to save confirmation for ' + user);
    done();
  });

  function assertErrorOnSaveForRegisteredUser(user, message) {
    assert(callbackCalledWithError(message), 'Error message not returned through callback.  Expected message is ' + message);
  }
  
  function assertErrorOnGetForRegisteredUser(user, message) {
    assert(callbackCalledWithError(message), 'Error message not returned through callback.  Expected message is ' + message);
  }
