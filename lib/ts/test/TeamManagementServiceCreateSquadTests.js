...
describe("Team Management Service - Create Squad Unit Tests", function () {
    // 1. Module Setup
    var levelimdb,
        tms,
        spyCallback,
    ...
    function injectDependenciesForUserManagementService() {
        ...
        $jsInject.register("TeamManagementService", ["clubs", "squads", teammanagementservice]);
    ...
    function assertSquadWasCreated() {
        assert(stubPut.calledWith(squadname + '~' + season, { agelimit: agelimit, admin: adminemail }, { sync: true }), 'put not called with correct parameters');
